/**
 * InstagramService
 * MediaService object for Instagram.  Note that we use a top level hash to keep track of
 * the real time streaming subscriptions.  We do this because we need to be able to find
 * our way back to this object from the code that runs when Instagram issues a POST
  ...
  instagram.tags.recent({
    ...
    complete: function(data, pagination) {
      ...
      data.reverse().forEach(function(item) {
        if (item.type === 'image') {
          // Wedding hack - hard code a list of users to ignore
<<<<<<< HEAD
=======
          console.log(item.user.username);
>>>>>>> b45087fd9b93fced40345cea49feeb5c9ca09688
          if (item.user.username != 'alohomora03' &&
            item.user.username != 'jennicams' &&
            item.user.username != 'gcairo007') {
