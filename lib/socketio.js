  ...
  socket.on('start', function(data) {
    ...
    if (streamTwitter && socket.handshake.user.twitterToken) {
      ...
      twitter.get('/search/tweets.json', {q: '#' + keyword, count: maxSlides}, function(err, data) {
        ...
        pushInitialMedia(initialMedia, socket, maxSlides, twitterReceived, instagramReceived);
      });
      ...
      // Pretend that we've already received initial media for twitter
      twitterReceived = true;
    }
    ...
    if (streamInstagram && socket.handshake.user.instagramAccessToken) {
      ...
      // Pretend that we've already received initial media for instagram
      instagramReceived = true;
    }
  });
...
function pushInitialMedia(array, socket, maxSlides, twitter, instagram) {
